...
The html logfile allows to manually verify that the correct photos have been
...
Should the structural similarity score be below this threshold it will be shown
in red in the logfile as seen above and the photo will not be annotated with a
location on iNaturalist and the caption will not be updated.
...
This code has been written in Python3. It uses f-strings and hence needs Python
...
slow by high-quality picture comparisons. Possible values are `small`, `medium`,
`large` and `original`. This setting is a tradeoff between runtime and quality
of results. If this size is lowered, the structural similarity score needs to be
lowered as well, e.g. `SSIM_INATSIZE = 'small'` and `SSIM_THRESHOLD = 0.8` is a
